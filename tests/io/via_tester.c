...
void test_t1_pb7_on_off(void)
{
    ...
    /* Make sure PB7 is input and low for comparison later. */
    via_write(via, 0x2, 0);
    via_write(via, 0x0, 0);

    /* Enable T1 PB7 output, which should result in a callback
     * and PB7 going high. */
    via_write(via, 0xB, 0x80);

    TEST_ASSERT_EQUAL(1, events.numEvents);
    TEST_ASSERT_EQUAL(VIA_PORTB, events.events[0].data.port);

    TEST_ASSERT_BIT_HIGH(7, via_read_data_port(via, false));

    /* Disable PB7 and make sure we get an event and it goes back to input. */
    events.numEvents = 0;
    via_write(via, 0xb, 0);
    TEST_ASSERT_EQUAL(1, events.numEvents);
    TEST_ASSERT_EQUAL(VIA_PORTB, events.events[0].data.port);
    TEST_ASSERT_BIT_LOW(7, via_read_data_port(via, false));

    /* Make sure the pin is still input by changing the input side and reading it back. */
    via_write_data_port(via, false, 0x80, 0x80);
    TEST_ASSERT_BIT_HIGH(7, via_read_data_port(via, false));

    /* Set port back to output and drive it low. */
    via_write(via, 0x0, 0x00);
    via_write(via, 0x2, 0x80);

    events.numEvents = 0;

    /* Enable PB7 output again and make sure it overrides the PORTB output. */
    via_write(via, 0xB, 0x80);
    TEST_ASSERT_EQUAL(1, events.numEvents);
    TEST_ASSERT_EQUAL(VIA_PORTB, events.events[0].data.port);
    TEST_ASSERT_BIT_HIGH(7, via_read_data_port(via, false));
...
void test_t1_pb7_oneshot(void)
{
    ...
    /* Configure for Oneshot + PB7. */
    via_write(via, 0xB, 0x80);

    /* Don't care about events generated by configuration. */
    events.numEvents = 0;
    ...
    /* Make sure PB7 went high. */
    TEST_ASSERT_EQUAL(2, events.numEvents);
    TEST_ASSERT_EQUAL(VIA_PORTB, events.events[1].data.port);
    TEST_ASSERT_BIT_HIGH(7, via_read_data_port(via, false));

    /* Make sure letting the timer keep running does not re-trigger. */
    emu_tick(emu);
    emu_tick(emu);
    emu_tick(emu);
    emu_tick(emu);
...
void test_t1_pb7_cont(void)
{
    ...
    /* Configure for Continuous + PB7. */
    via_write(via, 0xB, 0xC0);
    ...
    /* Expiration should take 3+1.5 cycles. Tick 4 times and make sure it hasn't
     * expired. */
    emu_tick(emu);
    emu_tick(emu);
    emu_tick(emu);
    ...
    /* Make sure PB7 hasn't changed. */
    TEST_ASSERT_EQUAL(0, events.numEvents);
    TEST_ASSERT_BIT_LOW(7, via_read_data_port(via, false));
    ...
    /* PB7 should have toggled back low. */
    TEST_ASSERT_EQUAL(2, events.numEvents);
    TEST_ASSERT_EQUAL(VIA_PORTB, events.events[1].data.port);
    TEST_ASSERT_BIT_LOW(7, via_read_data_port(via, false));
}
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_t1_pb7_on_off);
    RUN_TEST(test_t1_pb7_oneshot);
    RUN_TEST(test_t1_pb7_cont);
